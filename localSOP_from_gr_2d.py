...
big_x = np.load(path+af+'/'+'big_af-'+af+'_x.npy')*(145/512)/sigma_big
big_y = np.load(path+af+'/'+'big_af-'+af+'_y.npy')*(145/512)/sigma_big
small_x = np.load(path+af+'/'+'small_af-'+af+'_x.npy')*(145/512)/sigma_big
small_y = np.load(path+af+'/'+'small_af-'+af+'_y.npy')*(145/512)/sigma_big
...
#Plot the mean local pair fucntion to get the limits of integrations, here I am doing the integtration only over the 1st peak
#In this calculation, we have used single cutoff for the g(r) as it dosen't vary much with the density of the liquid.
#The variation is within +-0.1. So I have kept the mean of those cutoff same for all.
...
for f in range(frames):
	...
	pgr_ab = particle_gr(big_x[:,f],big_y[:,f],small_x[:,f],small_y[:,f],Na,Nb)
	...
	r_edges = pgr_aa[int(0.86/delr):int(1.4/delr),-1]    
...
# 	plt.figure()
# 	r = pgr_bs[:,-1]
# 	gr_bs = np.delete(pgr_bs,-1,axis=1)
# 	gr_bs = np.mean(gr_bs,axis=1)
# 	plt.plot(r,gr_bs)
# 	plt.show()
...
# 	plt.figure()
# 	r = pgr_sb[:,-1]
# 	gr_sb = np.delete(pgr_sb,-1,axis=1)
# 	gr_sb = np.mean(gr_sb,axis=1)
# 	plt.plot(r,gr_sb)
# 	plt.show()
...
# 	plt.figure()
# 	r = pgr_ss[:,-1]
# 	gr_ss = np.delete(pgr_ss,-1,axis=1)
# 	gr_ss = np.mean(gr_ss,axis=1)
# 	plt.plot(r,gr_ss)
# 	plt.show()
...
# 	print(np.shape(bulk))
	...
	dist_bulk, idxs = tree.query(bulk_df, k=40, distance_upper_bound= 2.5)
...
#np.savetxt('Phi_t=0-100_af=73.txt',Phi)
