...
%strain.    For quescient systems call functions with _qst and for sheared
%systems it is without qst.
...
for i = 3 : 2 : 24
    ...
    t2 = i
    eps = Dmin_3D_opt([X(:,t1) Y(:,t1) Z(:,t1)],[X(:,t2) Y(:,t2) Z(:,t2)]);  % if sheared system then Dmin_3D_opt
    ...
    averageEps = averagelocalstrain([X(:,t2) Y(:,t2) Z(:,t2)],eps);     % if sheared system then averagelocalstrain
